...
func TestParseCompare(t *testing.T) {
	for _, fixture := range []struct {
		...
		{"<10", TYPE_CLUE_INT, 3, &Compare{
			Operation: COMPARE_LESS,
			IntValue:  int64(10),
		}},
		{"< 10", TYPE_CLUE_INT, 4, &Compare{
			Operation: COMPARE_LESS,
			IntValue:  int64(10),
		}},
		{"0x01000007", TYPE_CLUE_INT, 10, &Compare{
		...
		{"D6E229D3-35DA-11D1-9034-00A0C90349BE", TYPE_CLUE_STRING, 36, &Compare{
			Operation:   COMPARE_EQUAL,
			StringValue: "D6E229D3-35DA-11D1-9034-00A0C90349BE",
		}},
		{`Invalid\ partition\ table		english`, TYPE_CLUE_STRING, 25, &Compare{
			Operation:   COMPARE_EQUAL,
			StringValue: "Invalid partition table",
		}},
	} {
		c, s, err := ParseCompare(fixture.line, Clue(fixture.clue))
		assert.NoError(t, err)
